...
| 变量 | 说明 | 必需 |
|------|------|------|
| `NUGET_API_KEY` | **远程推送时使用**。支持两种值：<br>1. **Trusted Publishing (OIDC)**：由 `NuGet/login@v1` action 输出的 1 小时短期 token（GitHub Actions 推荐方式）<br>2. **Legacy API Key**：nuget.org 长期 API Key（本地手动推送或 OIDC 回退时使用） | 远程推送时必需；`--local` 模式不需要 |
...
- **GitHub Actions（Trusted Publishing 推荐）**：
  ...
  ```
  推送时使用 `${{ steps.login.outputs.NUGET_API_KEY }}`（1 小时临时 token）。
...
- **本地手动推送**：
  ```bash
  export NUGET_API_KEY=oy2xxx...  # 从 nuget.org/account/apikeys 获取
  dotnet nuget push ... -k "$NUGET_API_KEY"
  ```

> **注意**：`zl-pipeline.py` 的 `do_push` 函数读取环境变量 `NUGET_API_KEY`，缺失时仅打印警告并跳过远程推送（本地缓存仍执行）。`publish_all.py` 不直接处理 API Key，推送由 GitHub Actions workflow 中的 shell 脚本完成。
